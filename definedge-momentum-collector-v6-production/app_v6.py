...
def home_v6():
    legacy.cleanup_jobs(); return render_template("index_v6.html",**_template_kwargs(otp_sent=False))
...
def send_otp_route_v6():
    try:
        ...
        response.set_cookie("de_state",state_id,max_age=legacy.OTP_TTL_SECONDS,httponly=True,secure=True,samesite="Lax"); return response
    except Exception as exc:
        return _render_index(False,error=str(exc),status_code=400)

def resend_otp_route_v6():
    try:
        legacy.cleanup_states(); state_id=request.cookies.get("de_state", ""); state=legacy.OTP_STATES.get(state_id)
        ...
        data=response.json(); otp_token=data.get("otp_token")
        if not otp_token:
            raise RuntimeError("Definedge returned success but no otp_token for the fresh OTP.")
        state["otp_token"]=otp_token; state["created"]=time.time(); state["password_verified"]=True
        ...
        page.set_cookie("de_state",state_id,max_age=legacy.OTP_TTL_SECONDS,httponly=True,secure=True,samesite="Lax"); return page
    except Exception as exc:
        return _render_index(False,error=str(exc),status_code=400)

def authenticate_v6(state_id,otp):
    legacy.cleanup_states(); state=legacy.OTP_STATES.get(state_id)
    if not state or not state.get("password_verified"):
        raise RuntimeError("OTP session expired. Use Send Fresh OTP or start again with the collector password.")
    otp_code=str(otp or "").strip(); otp_token=str(state.get("otp_token") or "").strip()
    if not otp_code: raise RuntimeError("Enter the Definedge OTP.")
    if not otp_token: raise RuntimeError("OTP token is missing. Click Send Fresh OTP.")
    ...
    data=response.json(); session_key=data.get("api_session_key") or legacy.extract_session_key(data)
    if not session_key: raise RuntimeError("Definedge authentication succeeded but no usable api_session_key was returned.")
    legacy.OTP_STATES.pop(state_id,None); return session_key
...
def job_worker_v6(job_id,session_key,input_bytes,input_filename,before_minutes,after_minutes,include_chain,source_meta=None):
    try:
        ...
        with legacy.JOB_LOCK: job=dict(legacy.JOBS.get(job_id,{}))
        output_path=job.get("output_path")
        if not output_path: raise RuntimeError("Reconstruction completed but no evidence ZIP was produced.")
        source_path=Path(output_path); v6_name=source_path.name.replace("Momentum_Full_Analysis_V5","Momentum_Full_Evidence_V6"); target_path=source_path.with_name(v6_name)
        if target_path!=source_path:
            source_path.replace(target_path); output_path=str(target_path); legacy.set_job(job_id,output_path=output_path,output_filename=target_path.name)
        ...
        try:
            ...
            with legacy.JOB_LOCK: latest_job=dict(legacy.JOBS.get(job_id,{}))
            summary=dict(latest_job.get("summary",{})); summary.update({"pipeline_version":"6.3","reconstruction_engine":"V5 frozen","ready_for_trading_brain":False,"local_evidence_ready":True,"drive_auth_mode":drive_auth_mode(),"drive_publish_error":str(publish_exc),"source_input":input_filename})
            legacy.set_job(job_id,status="error",progress=100,message=f"Evidence package is complete, but Drive publication failed: {publish_exc} Download Evidence ZIP below; Definedge collection does not need to be rerun.",summary=summary,output_path=output_path,output_filename=Path(output_path).name); return
        with legacy.JOB_LOCK: latest_job=dict(legacy.JOBS.get(job_id,{}))
        summary=dict(latest_job.get("summary",{})); summary.update({"drive_file_id":uploaded.get("id",""),"drive_file_name":uploaded.get("name",""),"drive_file_url":uploaded.get("webViewLink",""),"pipeline_version":"6.3","reconstruction_engine":"V5 frozen","ready_for_trading_brain":True,"local_evidence_ready":True,"drive_auth_mode":drive_auth_mode(),"manifest_status":manifest.get("status",""),"source_input":input_filename})
...
def collect_route_v6():
    try:
        ...
        if missing: raise RuntimeError("V6 Drive publishing is not configured: "+", ".join(missing))
        otp=request.form.get("otp", "").strip(); state_id=request.cookies.get("de_state", "")
        if not otp: raise RuntimeError("Enter the Definedge OTP.")
        ...
        if not input_bytes: raise RuntimeError("Uploaded AlgoStra ZIP is empty.")
        test_zf,_=legacy.safe_zip_input(input_bytes)
        csv_names=[n for n in test_zf.namelist() if n.lower().endswith(".csv") and not n.endswith("/")]; test_zf.close()
        if len(csv_names)!=24:
        ...
        before_minutes=max(0,min(60,int(request.form.get("before_min","3")))); after_minutes=max(0,min(180,int(request.form.get("after_min","15"))); include_chain=request.form.get("include_chain")=="yes"
        session_key=authenticate_v6(state_id,otp); job_id=secrets.token_urlsafe(18)
        ...
        response=make_response(render_template("job_v6.html",job_id=job_id)); response.delete_cookie("de_state"); return response
    except Exception as exc:
...
def status_route_v6(job_id):
    ...
    with legacy.JOB_LOCK:
        ...
        if not job: return jsonify({"status":"missing","progress":100,"message":"This job expired or the service restarted."}),404
        snapshot=dict(job)
    output_path=snapshot.get("output_path"); return jsonify({"status":snapshot.get("status","missing"),"progress":snapshot.get("progress",0),"message":snapshot.get("message",""),"summary":dict(snapshot.get("summary",{})),"download_ready":bool(output_path and Path(output_path).exists())})

def download_route_v6(job_id):
    legacy.cleanup_jobs()
    with legacy.JOB_LOCK:
        job=legacy.JOBS.get(job_id)
        if not job: return "Job expired.",404
        output_path=job.get("output_path"); output_filename=job.get("output_filename") or "Definedge_Evidence_V6.zip"
    if not output_path: return "Output file unavailable.",404
    path=Path(output_path)
    if not path.exists(): return "Output file expired.",404
    return send_file(path,mimetype="application/zip",as_attachment=True,download_name=output_filename,max_age=0)

def health_v6():
    missing=_drive_config_status(); return {"status":"ok" if not missing else "configuration_incomplete","version":"6.3","reconstruction_engine":"V5 frozen","input_mode":"explicit_manual_zip","drive_configured":not bool(missing),"drive_auth_mode":drive_auth_mode(),"drive_oauth_write_ready":oauth_write_configured(),"missing_configuration":missing}
...
if "resend_otp_route" in app.view_functions: app.view_functions["resend_otp_route"]=resend_otp_route_v6
else: app.add_url_rule("/resend-otp",endpoint="resend_otp_route",view_func=resend_otp_route_v6,methods=["POST"])
