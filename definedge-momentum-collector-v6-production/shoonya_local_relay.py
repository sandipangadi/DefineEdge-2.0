...
def fetch_nifty(factor2, lookback_days):
    ...
    if series_status >= 400 or not isinstance(series, list):
        ...
        raise RuntimeError("Shoonya 5-minute history failed: " + str(message))

    bars = [safe_bar(row) for row in series if safe_bar(row)]
    return {
        "status": "ok",
        ...
        "historical_5m": {"lookback_days": days, "bar_count": len(bars), "bars": bars},
...
<label for="lookback_days">Historical lookback days (1–30)</label><br><input id="lookback_days" name="lookback_days" type="number" value="7" min="1" max="30"><br>
